...
// ── Step 3: Download image buffer directly from Green API ───────────────────
// Green API's downloadFile endpoint returns the actual file bytes,
// not a URL. We POST with credentials and get the file back directly.
async function fetchImageUrlFromGreenApi(chatId, idMessage) {
  ...
  // Try Method 1: downloadFile — returns file bytes directly
  try {
    ...
    // If we got bytes back, convert to base64 directly and return as dataUrl
    if (res.data && res.data.byteLength > 0) {
      const buffer   = Buffer.from(res.data);
      const mime     = detectMimeType(buffer);
      const base64   = buffer.toString('base64');
      const dataUrl  = `data:${mime};base64,${base64}`;
      logger.info('[Vision] downloadFile success — got bytes', {
        sizeKB: Math.round(buffer.length / 1024),
        mime,
      });
      // Return special object to signal we already have base64
      return { dataUrl, direct: true };
    }
  } catch (err) {
    logger.warn('[Vision] downloadFile (arraybuffer) failed', {
      status: err?.response?.status,
      error: err.message,
    });
  }

  // Try Method 2: downloadFile as JSON — may return a URL
  try {
    const res = await axios.post(
      `${BASE}/waInstance${INST}/downloadFile/${TOK}`,
      { chatId, idMessage },
