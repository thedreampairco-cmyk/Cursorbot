...
// ── Step 3: Fetch image URL from Green API using message ID ──────────────────
async function fetchImageUrlFromGreenApi(chatId, idMessage) {
  ...
  logger.info('[Vision] Fetching image URL from Green API...', { idMessage });

  // Small delay — Green API needs a moment to process the file
  ...
  logger.info('[Vision] Green API file response', {
...
async function matchSneakerFromImage(chatId, idMessage, imageUrl = null, jpegThumb = null) {
  ...
  // ── Try to get image URL from Green API if not in payload ──
  if (!imageUrl && idMessage && chatId) {
    try {
      imageUrl = await fetchImageUrlFromGreenApi(chatId, idMessage);
    } catch (err) {
  ...
  if (imageUrl) {
    try {
      ...
      logger.info('[Vision] Image downloaded and converted to base64', {
        sizeKB: Math.round(buffer.length / 1024),
      });
    } catch (err) {
      logger.warn('[Vision] Image download failed', { error: err.message });
    }
  }
