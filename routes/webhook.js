...
router.post('/', asyncHandler(async (req, res) => {
  ...
  if (msgType === "textMessage") {
    ...
    imageUrl = msgData?.imageMessageData?.downloadUrl
            ...
            || msgData?.imageMessageData?.jpegThumbnail
            || null;
    logger.info("[Webhook] Image received", { chatId, hasUrl: !!imageUrl });
  ...
  if (isImage) {
    ...
    if (matches.length) {
      await sendText(chatId, identified ? `Looks like a *${identified}* vibe! Closest matches 👟` : "Here are similar sneakers 👟");
      await sendProductImages(chatId, matches, 4);
      await sendText(chatId, matches.map((p, i) => `${i + 1}. ${p.name} – Rs.${p.price}`).join('\n'));
      client.lastMentionedProduct = { productId: String(matches[0].id), productName: matches[0].name, price: matches[0].price, imageUrl: matches[0].imageUrl || null };
