...
router.post('/', asyncHandler(async (req, res) => {
  ...
  if (msgType === "textMessage") {
    ...
    // Fetch real download URL from Green API using idMessage
    if (!imageUrl && idMessage) {
      try {
        ...
        // Green API requires a small delay before file is ready
        await new Promise((r) => setTimeout(r, 2000));

        logger.info("[Webhook] Calling getFileByIdMessage", { idMessage, chatId });
        ...
        logger.info("[Webhook] getFileByIdMessage raw response", {
          status: dlRes.status,
          keys: dlRes.data ? Object.keys(dlRes.data) : [],
          data: JSON.stringify(dlRes.data).slice(0, 300)
        });
        ...
        imageUrl = dlRes.data?.downloadUrl
                ...
                || dlRes.data?.url
                || dlRes.data?.urlFile
                || dlRes.data?.fileNameDownload
                || null;

      } catch (dlErr) {
        logger.error("[Webhook] getFileByIdMessage failed", {
          error: dlErr.message,
          status: dlErr?.response?.status,
          data: JSON.stringify(dlErr?.response?.data).slice(0, 200)
        });
      }
    }
