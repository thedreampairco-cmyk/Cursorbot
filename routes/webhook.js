...
router.post('/', asyncHandler(async (req, res) => {
  ...
  if (msgType === "textMessage") {
    ...
    // Fallback: use Green API downloadFile endpoint to get the URL
    if (!imageUrl && msgData?.idMessage) {
      try {
        ...
        imageUrl = dlRes.data?.downloadUrl || dlRes.data?.fileUrl || null;
        logger.info("[Webhook] Image URL from downloadFile", { imageUrl: !!imageUrl });
      } catch (dlErr) {
        logger.warn("[Webhook] downloadFile fallback failed", { error: dlErr.message });
      }
    }

    logger.info("[Webhook] Image received", { chatId, hasUrl: !!imageUrl, idMessage: msgData?.idMessage });
