...
router.post('/', asyncHandler(async (req, res) => {
  ...
  if (msgType === "textMessage") {
    ...
    // Try every possible field Green API might use for the image URL
    imageUrl = msgData?.imageMessageData?.downloadUrl
            || msgData?.imageMessageData?.url
            || msgData?.imageMessageData?.fileUrl
            || msgData?.imageMessageData?.mediaUrl
            || msgData?.imageMessageData?.linkToFile
            || null;

    // Fallback: use Green API getFileByIdMessage to retrieve the download URL
    if (!imageUrl && msgData?.idMessage) {
      try {
        const { greenApiClient } = require('../config/api');
        const env2 = require('../config/env');

        // Method 1: getFileByIdMessage
        const dlRes = await greenApiClient.post(
          `/getFileByIdMessage/${env2.greenApi.token}`,
          { chatId, idMessage: msgData.idMessage }
        );
        imageUrl = dlRes.data?.downloadUrl
                || dlRes.data?.fileUrl
                || dlRes.data?.url
                || null;
        logger.info("[Webhook] Image URL via getFileByIdMessage", { found: !!imageUrl, data: dlRes.data });

        // Method 2: try downloadFile if Method 1 failed
        if (!imageUrl) {
          const dl2 = await greenApiClient.post(
            `/downloadFile/${env2.greenApi.token}`,
            { chatId, idMessage: msgData.idMessage }
          );
          imageUrl = dl2.data?.downloadUrl || dl2.data?.fileUrl || dl2.data?.url || null;
          logger.info("[Webhook] Image URL via downloadFile", { found: !!imageUrl, data: dl2.data });
        }

      } catch (dlErr) {
        logger.warn("[Webhook] Image URL fetch failed", {
          error: dlErr.message,
          response: dlErr?.response?.data
        });
      }
    }
    ...
    logger.info("[Webhook] Image received", {
      ...
      imageKeys: msgData?.imageMessageData ? Object.keys(msgData.imageMessageData) : [],
      msgDataKeys: msgData ? Object.keys(msgData) : []
    });
  ...
  if (isImage) {
    ...
    const { identified, matches, noUrl } = await matchSneakerFromImage(imageUrl);
