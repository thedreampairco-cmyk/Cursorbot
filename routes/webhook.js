...
router.post('/', asyncHandler(async (req, res) => {
  ...
  if (msgType === "textMessage") {
    ...
    // Store base64 thumbnail as absolute last resort
    jpegThumb = imgData.jpegThumbnail || null;

    // Fallback: use Green API getFileByIdMessage to get real download URL
    if (!imageUrl && msgData?.idMessage) {
      try {
        const axios = require('axios');
        const env2 = require('../config/env');
        const BASE = env2.greenApi.baseUrl || 'https://api.greenapi.com';
        const INST = env2.greenApi.instanceId;
        const TOK  = env2.greenApi.token;

        logger.info("[Webhook] Fetching image via getFileByIdMessage", { idMessage: msgData.idMessage });
        ...
        logger.warn("[Webhook] getFileByIdMessage failed", {
          ...
          response: JSON.stringify(dlErr?.response?.data)
        });
      }
    }
    ...
    logger.info("[Webhook] Image received", {
      ...
      RAW_imageMessageData: JSON.stringify(msgData?.imageMessageData || {}).slice(0, 300)
    });
